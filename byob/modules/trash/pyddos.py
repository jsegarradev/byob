...
def add_useragent():
	user_agent_mock = ["Mozilla/5.0 (Windows NT 6.4; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2225.0 Safari/537.36"]
	return user_agent_mock
...
'''
	...
	options.add_argument('-p',metavar='<int>',default=80,help='Specify port target (default = 80)' + ' |Only required with pyslow attack|')
	...
	if args.Synflood:
		uid = os.getpid()
		# if uid == 0:
		# 	sys.exit('[*] You have enough permisson to run this script')
		# 	time.sleep(0.5)
		# else:
		...
		tgt = args.d
		threads = []
		print ('[*] Start send request to: ' + tgt)
		while 1:
			try:
